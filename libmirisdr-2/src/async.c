...
int mirisdr_cancel_async_now (mirisdr_dev_t *p) {
    ...
    while ((p->async_status != MIRISDR_ASYNC_INACTIVE) &&
           (p->async_status != MIRISDR_ASYNC_FAILED)) usleep(20000);
...
int mirisdr_read_async (mirisdr_dev_t *p, mirisdr_read_async_cb_t cb, void *ctx, uint32_t num, uint32_t len) {
    ...
    fprintf(stderr, "async read on device %u, buffers: %zu, output size: ", p->index, p->xfer_buf_num);
    if (p->xfer_out_len) {
        fprintf(stderr, "%zu", p->xfer_out_len);
    } else {
        fprintf(stderr, "auto");
    }
