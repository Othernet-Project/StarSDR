/*
 * Star_fm, recieve FM using your usb sdr 
 *
...
void rotate_90(int16_t *buf, uint32_t len)
...
starsdr_int32 starsdr_callback(void *ctx, starsdr_int16 *buf, starsdr_int32 num_samples)
{
	int i;
	...
	memcpy(d->lowpassed, s->buf16, 4*num_samples);
...
static void *dongle_thread_fn(void *arg)
{
	...
	starsdr_start_rx(s->dev, starsdr_callback, s, s->buf_len/4);
	return 0;
}
...
static void *controller_thread_fn(void *arg)
{
	...
	while (!do_exit) {
		...
		starsdr_set_rx_frequency(dongle.dev, dongle.freq);
...
int main(int argc, char **argv)
{
	...
	dongle.dev = starsdr_open_device(dongle.dev_index);
	if (!dongle.dev) {
		fprintf(stderr, "Failed to open starsdr device #%d.\n", dongle.dev_index);
		exit(1);
	}
#ifndef _WIN32
